...
def run(hmm_estimate,num_states,filename,length_vec,root_path,multiple,species_name,
    ...
    if method_mode == 0:
        ...
        filename3 = "%s/estimate_enh%d_bm_5k_%d_s%d"%(path_1, run_id, n_components1, version)
        np.savetxt(filename3, state_est, fmt='%d', delimiter='\t')
        filename1 = "%s/branch_enh%d_bm_5k_%d_s%d"%(path_1, run_id, n_components1, version)
        np.savetxt(filename1, tree1.branch_params, fmt='%.6f', delimiter='\t')
        filename1 = "%s/means_enh%d_bm_5k_%d_s%d"%(path_1, run_id, n_components1, version)
        np.savetxt(filename1, tree1.means_, fmt='%.6f', delimiter='\t')
        filename1 = "%s/trans_enh%d_bm_5k_%d_s%d"%(path_1, run_id, n_components1, version)
        np.savetxt(filename1, tree1.transmat_, fmt='%.6f', delimiter='\t')
        filename1 = "%s/start_enh%d_bm_5k_%d_s%d"%(path_1, run_id, n_components1, version)
        ...
        # the second test
        filename3 = "%s/estimate_ou_enh%d_1a1_%.2f_%d_s%d"%(path_1, run_id, lambda_0, n_components1, version)
        state_est = tree1.predict(x,len_vec)
        np.savetxt(filename3, state_est, fmt='%d', delimiter='\t')
        # the second test
        filename1 = "%s/means_ou_enh%d_1a1_%.2f_%d_s%d"%(path_1, run_id, lambda_0, n_components1, version)
        np.savetxt(filename1, tree1.params_vec1, fmt='%.6f', delimiter='\t')
        filename1 = "%s/trans_ou_enh%d_1a1_%.2f_%d_s%d"%(path_1, run_id, lambda_0, n_components1, version)
        np.savetxt(filename1, tree1.transmat_, fmt='%.6f', delimiter='\t')
        filename1 = "%s/start_ou_enh%d_1a1_%.2f_%d_s%d"%(path_1, run_id, lambda_0, n_components1, version)
        np.savetxt(filename1, tree1.startprob_, fmt='%.6f', delimiter='\t')
